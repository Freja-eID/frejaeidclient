...
public class CustomIdentifierClientTest {
    ...
    @Test
    public void setCustomIdentifier_relyingPartyIdFromRequest_expectedSuccess() throws FrejaEidClientInternalException, FrejaEidException {
        customIdentifierClient = CustomIdentifierClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                .setHttpService(httpServiceMock)
                .build();
        ...
        for (SetCustomIdentifierRequest request : requests) {
            ...
            Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.CUSTOM_IDENTIFIER_SET, RequestTemplate.SET_CUSTOM_IDENITIFIER_TEMPLATE, request, EmptyFrejaResponse.class, RELYING_PARTY_ID);
        }

    }

    @Test
    public void setCustomIdentifier_customIdentifierAlreadySetForUser_expectCustomIdentifierAlreadySetError() throws FrejaEidClientInternalException, FrejaEidException {
        SetCustomIdentifierRequest setCustomIdentifierRequest = SetCustomIdentifierRequest.createDefaultWithEmail(EMAIL, CUSTOM_IDENTIFIER);
        try {
            customIdentifierClient = CustomIdentifierClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                    .setHttpService(httpServiceMock)
                    .build();
            Mockito.when(httpServiceMock.send(Mockito.anyString(), Mockito.any(RequestTemplate.class), Mockito.any(RelyingPartyRequest.class), Mockito.eq(EmptyFrejaResponse.class), (String) Mockito.isNull()))
                    .thenThrow(new FrejaEidException(FrejaEidErrorCode.USER_MANAGEMENT_CUSTOM_IDENTIFIER_ALREADY_EXISTS.getMessage(), FrejaEidErrorCode.USER_MANAGEMENT_CUSTOM_IDENTIFIER_ALREADY_EXISTS.getCode()));
            customIdentifierClient.set(setCustomIdentifierRequest);
            Assert.fail("Test should throw exception!");
        } catch (FrejaEidException rpEx) {
            Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.CUSTOM_IDENTIFIER_SET, RequestTemplate.SET_CUSTOM_IDENITIFIER_TEMPLATE, setCustomIdentifierRequest, EmptyFrejaResponse.class, null);
    ...
    public void deleteCustomIdentifier_success() throws FrejaEidClientInternalException, FrejaEidException {
        customIdentifierClient = CustomIdentifierClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                .setHttpService(httpServiceMock)
                .build();
        DeleteCustomIdentifierRequest deleteCustomIdentifierRequest = DeleteCustomIdentifierRequest.create(CUSTOM_IDENTIFIER);
        Mockito.when(httpServiceMock.send(Mockito.anyString(), Mockito.any(RequestTemplate.class), Mockito.any(RelyingPartyRequest.class), Mockito.eq(EmptyFrejaResponse.class), (String) Mockito.isNull())).thenReturn(EmptyFrejaResponse.INSTANCE);
        customIdentifierClient.delete(deleteCustomIdentifierRequest);
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.CUSTOM_IDENTIFIER_DELETE, RequestTemplate.DELETE_CUSTOM_IDENTIFIER_TEMPLATE, deleteCustomIdentifierRequest, EmptyFrejaResponse.class, null);
    }

    @Test
    public void deleteCustomIdentifier_relyingPartyIdFromRequest_expectedSuccess() throws FrejaEidClientInternalException, FrejaEidException {
        customIdentifierClient = CustomIdentifierClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                .setHttpService(httpServiceMock)
                .build();
        DeleteCustomIdentifierRequest deleteCustomIdentifierRequest = DeleteCustomIdentifierRequest.create(CUSTOM_IDENTIFIER, RELYING_PARTY_ID);
        Mockito.when(httpServiceMock.send(Mockito.anyString(), Mockito.any(RequestTemplate.class), Mockito.any(RelyingPartyRequest.class), Mockito.eq(EmptyFrejaResponse.class), Mockito.anyString())).thenReturn(EmptyFrejaResponse.INSTANCE);
        customIdentifierClient.delete(deleteCustomIdentifierRequest);
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.CUSTOM_IDENTIFIER_DELETE, RequestTemplate.DELETE_CUSTOM_IDENTIFIER_TEMPLATE, deleteCustomIdentifierRequest, EmptyFrejaResponse.class, RELYING_PARTY_ID);
    }

    @Test
    public void deleteCustomIdentifier_unexistingCustomIdentifier_expectNoUserForCustomIdentifierError() throws FrejaEidClientInternalException, FrejaEidException {
        DeleteCustomIdentifierRequest deleteCustomIdentifierRequest = DeleteCustomIdentifierRequest.create(CUSTOM_IDENTIFIER);
        try {
            customIdentifierClient = CustomIdentifierClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                    .setHttpService(httpServiceMock)
                    .build();
            Mockito.when(httpServiceMock.send(Mockito.anyString(), Mockito.any(RequestTemplate.class), Mockito.any(RelyingPartyRequest.class), Mockito.eq(EmptyFrejaResponse.class), (String) Mockito.isNull()))
                    .thenThrow(new FrejaEidException(FrejaEidErrorCode.USER_MANAGEMENT_CUSTOM_IDENTIFIER_DOES_NOT_EXIST.getMessage(), FrejaEidErrorCode.USER_MANAGEMENT_CUSTOM_IDENTIFIER_DOES_NOT_EXIST.getCode()));
            customIdentifierClient.delete(deleteCustomIdentifierRequest);
            Assert.fail("Test should throw exception!");
        } catch (FrejaEidException rpEx) {
            Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.CUSTOM_IDENTIFIER_DELETE, RequestTemplate.DELETE_CUSTOM_IDENTIFIER_TEMPLATE, deleteCustomIdentifierRequest, EmptyFrejaResponse.class, null);
            Assert.assertEquals("Invalid error", 5001, rpEx.getErrorCode());
            Assert.assertEquals("Invalid error", "There is no user for given custom identifier.", rpEx.getLocalizedMessage());
        }
    }
