...
public class OrganisationIdClientDeleteOrganisationIdTest {
    ...
    public void deleteOrganisationId_success() throws FrejaEidClientInternalException, FrejaEidException {
        ...
        organisaionIdClient.delete(deleteOrganisationIdRequest);
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.ORGANISATION_ID_DELETE, RequestTemplate.DELETE_ORGANINSATION_ID_TEMPLATE, deleteOrganisationIdRequest, EmptyFrejaResponse.class, null);
    }

    @Test
    public void deleteOrganisationId_relyingPartyIdFromRequest_expectedSuccess() throws FrejaEidClientInternalException, FrejaEidException {
        OrganisationIdClientApi organisaionIdClient = OrganisationIdClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                .setHttpService(httpServiceMock)
                .build();
        ...
        organisaionIdClient.delete(deleteOrganisationIdRequest);
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.ORGANISATION_ID_DELETE, RequestTemplate.DELETE_ORGANINSATION_ID_TEMPLATE, deleteOrganisationIdRequest, EmptyFrejaResponse.class, RELYING_PARTY_ID);
    }

    @Test
    public void deleteOrganisationId_unexistingCustomIdentifier_expectNoUserForCustomIdentifierError() throws FrejaEidClientInternalException, FrejaEidException {
        DeleteOrganisationIdRequest deleteOrganisationIdRequest = DeleteOrganisationIdRequest.create(IDENTIFIER);
        try {
            OrganisationIdClientApi organisaionIdClient = OrganisationIdClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                    .setHttpService(httpServiceMock)
