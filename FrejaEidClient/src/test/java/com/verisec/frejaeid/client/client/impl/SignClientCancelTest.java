...
public class SignClientCancelTest {
    ...
    public void cancelSign_relyingPartyIdNull_success() throws FrejaEidClientInternalException, FrejaEidException {
        ...
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.SIGN_CANCEL, RequestTemplate.CANCEL_SIGN_TEMPLATE, cancelSignRequest, EmptyFrejaResponse.class, null);
    }
    ...
    public void cancelSignPersonal_success() throws FrejaEidClientInternalException, FrejaEidException {
        ...
        signClient.cancel(cancelSignRequest);
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.SIGN_CANCEL, RequestTemplate.CANCEL_SIGN_TEMPLATE, cancelSignRequest, EmptyFrejaResponse.class, RELYING_PARTY_ID);
    }
    ...
    public void cancelSignOrganisational_success() throws FrejaEidClientInternalException, FrejaEidException {
        ...
        Mockito.when(httpServiceMock.send(Mockito.anyString(), Mockito.any(RequestTemplate.class), Mockito.any(RelyingPartyRequest.class), Mockito.eq(EmptyFrejaResponse.class), Mockito.anyString())).thenReturn(EmptyFrejaResponse.INSTANCE);
        signClient.cancel(cancelSignRequest);
        Mockito.verify(httpServiceMock).send(FrejaEnvironment.TEST.getUrl() + MethodUrl.ORGANISATION_SIGN_CANCEL, RequestTemplate.CANCEL_SIGN_TEMPLATE, cancelSignRequest, EmptyFrejaResponse.class, RELYING_PARTY_ID);
    }

    @Test
    public void cancelSign_invalidReference_expectErrorInvalidReference() throws FrejaEidClientInternalException, FrejaEidException {
        CancelSignRequest cancelSignRequest = CancelSignRequest.create("123");
        try {
            SignClientApi signClient = SignClient.create(SslSettings.create(TestUtil.getKeystorePath(TestUtil.KEYSTORE_PATH), TestUtil.KEYSTORE_PASSWORD, TestUtil.getKeystorePath(TestUtil.CERTIFICATE_PATH)), FrejaEnvironment.TEST)
                    .setHttpService(httpServiceMock)
                    .setTransactionContext(TransactionContext.PERSONAL).build();
            Mockito.when(httpServiceMock.send(Mockito.anyString(), Mockito.any(RequestTemplate.class), Mockito.any(RelyingPartyRequest.class), Mockito.eq(EmptyFrejaResponse.class), (String) Mockito.isNull())).thenThrow(new FrejaEidException(FrejaEidErrorCode.INVALID_REFERENCE.getMessage(), FrejaEidErrorCode.INVALID_REFERENCE.getCode()));
